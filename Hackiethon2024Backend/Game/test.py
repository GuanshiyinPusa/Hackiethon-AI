...
def validMove(moveset, player):
    ...
    # check if out of bound
        ...
        player._xCoord + player._direction * moveset[0]
    ) > RIGHTBORDER:
        if moveset[1]:
            return True
        else:
...
def check_move_collision(player1, player2, cached_move_1, cached_move_2):
    return (
        ...
        and player2._xCoord + cached_move_2[0] == player1._xCoord
    )
...
def playerInfo(player, playerName, action):
    print(
        f"{playerName} POS: {player._xCoord, player._yCoord}, {player._hp}, midair: {player._midair}, blocking: {player._blocking, player._block._shieldHp}, stun: {player._stun}, facing: {player._direction}, airvelo:{player._velocity}"
    )
    print(
        f"             SPEED: {player._speed}, ATKBUFF: {player._atkbuff}, DURATION: {player._currentBuffDuration}"
    )
