...
def updateMidair(player):
    ...
    if player._midair:
        if player._falling:
            ...
            if (
                player.get_past_move(check_point)[1] not in ((0, 1), (1, 1), (-1, 1))
            ) or player._airvelo == 0:
...
def projectile_move(
    projectiles, knock1, stun1, knock2, stun2, player1, player2, p1_dict, p2_dict
):
    ...
    for proj_index in range(len(projectiles)):
        ...
        for nextProjNum in range(len(projectiles)):
            ...
            if nextProj:
                ...
                if nextproj_obj._id != proj_obj._id and proj_obj._checkProjCollision(
                    nextproj_obj
                ):
