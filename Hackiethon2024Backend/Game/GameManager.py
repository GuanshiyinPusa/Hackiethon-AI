...
def checkCollision(player1, player2, knock1, knock2, check_midair=False):
    if correct_dir_pos(player1, player2, knock1, knock2):
        ...
        if (
            (player1._yCoord == player2._yCoord)
...
def executeOneTurn(
    player1, player2, p1_script, p2_script, p1_json_dict, p2_json_dict, projectiles
):
    ...
    p1_projectiles = [
        ...
        if proj["projectile"]._player._id == 1
    ]
    p2_projectiles = [
        proj["projectile"]
        for proj in projectiles
        if proj["projectile"]._player._id == 2
    ]
    ...
    knock1, stun1, knock2, stun2, projectiles = performActions(
        player1, player2, act1, act2, stun1, stun2, projectiles
    )
...
def setupGame(p1_script, p2_script, leftstart=LEFTSTART, rightstart=RIGHTSTART):
    ...
    player1 = Player_Controller(
        leftstart, 0, HP, GORIGHT, *p1_script.init_player_skills(), 1
    )
    player2 = Player_Controller(
        rightstart, 0, HP, GOLEFT, *p2_script.init_player_skills(), 2
    )
...
def performActions(player1, player2, act1, act2, stun1, stun2, projectiles):
    ...
    if act1[0] not in (
        attack_actions.keys() | defense_actions.keys() | projectile_actions.keys()
    ):
    ...
    if isinstance(cached_move_1, list) and isinstance(cached_move_2, list):
        if (
            check_move_collision(player1, player2, cached_move_1, cached_move_2)
            and cached_move_1[1] == cached_move_2[1]
            and abs(player1._xCoord - player2._xCoord) == 1
        ):
            cached_move_1 = cached_move_2 = None
    ...
    if isinstance(cached_move_1, list):
        if (
            player1._xCoord + cached_move_1[0] == player2._xCoord
            and cached_move_2 in ([0, 0], None)
            and not cached_move_1[1]
        ):
            cached_move_1[0] = 0
        ...
        player1._moves[-1] = (
            "move",
            (cached_move_1[0] * player1._direction, cached_move_1[1]),
        )
    if isinstance(cached_move_2, list):
        if (
            player2._xCoord + cached_move_2[0] == player1._xCoord
            and cached_move_1 in ([0, 0], None)
            and not cached_move_2[1]
        ):
            cached_move_2[0] = 0
        ...
        player2._moves[-1] = (
            "move",
            (cached_move_2[0] * player2._direction, cached_move_2[1]),
        )
...
def startGame(path1, path2, submissionpath, roundNum):
    ...
    # Loops through turns
    while game_running:
        projectiles, p1_dead, p2_dead = executeOneTurn(
            player1,
            player2,
            p1_script,
            p2_script,
