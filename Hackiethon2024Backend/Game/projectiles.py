...
class Projectile:
    ...
    def __init__(self, player, path, size, type, trait, collision, timer, collisionHp):
    ...
    def _travel(self):
        if (
            0 < self._pathIndex < len(self._path)
            and LEFTBORDER < self._xCoord < RIGHTBORDER
        ):
        ...
        elif self._pathIndex >= len(self._path) or (
            (self._xCoord <= LEFTBORDER) or (self._xCoord >= RIGHTBORDER)
        ):
...
class ProjectileSkill(AttackSkill):
    def __init__(
        self, player, startup, cooldown, damage, blockable, knockback, stun, skillName
    ):
        AttackSkill.__init__(
            self,
            startup=startup,
            cooldown=cooldown,
            damage=damage,
...
class Hadoken(ProjectileSkill):
    ...
    def _activateSkill(self):
        ...
        projectile = self.summonProjectile(
            path=travelPath, size=(1, 1), trait=None, collision=True, timer=0, colHp=3
        )
        return [
            self._skillType,
            {
                "damage": self._skillValue,
                "blockable": self._blockable,
                "knockback": self._knockback,
                "stun": self._stun,
                "self_stun": self._stunself,
                "projectile": projectile,
            },
        ]
